    ...
    <main className="min-h-screen" style={{ backgroundColor: "#FDFAF7" }}>
      ...
      <header className="flex items-center justify-between px-6 py-4 max-w-5xl mx-auto">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xl font-bold" style={{ color: "#2D2A2E" }}>
            아이랑
          </span>
        </div>
        <nav className="flex items-center gap-3">
          ...
          <Link
            href="/signup"
            className="px-5 py-2 text-sm font-semibold rounded-full text-white transition-colors"
      ...
      <section className="px-6 py-20 text-center max-w-3xl mx-auto">
        ...
        <p className="text-lg leading-relaxed mb-10" style={{ color: "#5C5860" }}>
          임신 5주부터 40주까지, 아기의 성장과 엄마의 이야기를 기록하세요.
          <br />
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          <Link
            href="/signup"
            className="px-8 py-4 text-base font-semibold rounded-2xl text-white transition-all hover:opacity-90 shadow-lg"
            style={{ backgroundColor: "#FFB4A2" }}
          >
            무료로 시작하기 🌸
          </Link>
          <Link
            href="/demo"
            className="px-8 py-4 text-base font-semibold rounded-2xl transition-all hover:opacity-90"
      ...
      <section className="px-6 py-16 max-w-5xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="p-6 rounded-3xl shadow-sm" style={{ backgroundColor: "#FFFFFF" }}>
            ...
            <h3 className="text-lg font-semibold mb-2" style={{ color: "#2D2A2E" }}>
              40주 공식 가이드
            </h3>
      ...
      <section className="px-6 py-16 max-w-5xl mx-auto">
        <div className="p-8 rounded-3xl" style={{ backgroundColor: "#F5EBE0" }}>
          <h2 className="text-2xl font-bold mb-2" style={{ color: "#2D2A2E" }}>
            📅 40주 타임라인
          </h2>
          <p className="text-sm mb-8" style={{ color: "#5C5860" }}>
            임신 5주부터 40주까지, 매 주차의 이야기를 기록해요
          </p>
          ...
          <div className="flex gap-3 overflow-x-auto scrollbar-hide pb-2">
            {[5, 8, 12, 16, 20, 24, 28, 32, 36, 40].map((week) => (
              <div
                key={week}
                className="flex-shrink-0 w-20 h-24 rounded-2xl flex flex-col items-center justify-center gap-1 transition-all cursor-pointer hover:scale-105"
                style={{
                  backgroundColor: week === 12 ? "#FFB4A2" : "#FFFFFF",
                  color: week === 12 ? "#FFFFFF" : "#2D2A2E",
                }}
              >
                <span className="text-lg">{week === 12 ? "🍋" : week <= 12 ? "🌱" : week <= 20 ? "🍋" : week <= 28 ? "🍆" : week <= 36 ? "🍉" : "👶"}</span>
                <span className="text-xs font-medium">{week}주</span>
                {week === 12 && (
                  <span className="text-xs opacity-80">현재</span>
                )}
              </div>
            ))}
          </div>
      ...
      {/* Record Types */}
      <section className="px-6 py-16 max-w-5xl mx-auto">
        <h2 className="text-2xl font-bold text-center mb-2" style={{ color: "#2D2A2E" }}>
          ✏️ 5가지 기록
        </h2>
        <p className="text-center text-sm mb-10" style={{ color: "#5C5860" }}>
          일기부터 검사 기록까지, 임신의 모든 순간을 담아요
        </p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          {[
            ...
            <div
              key={item.label}
              className="p-4 rounded-2xl text-center transition-all hover:scale-105 cursor-pointer"
              style={{ backgroundColor: "#FFFFFF" }}
            >
              <div className="text-3xl mb-2">{item.icon}</div>
              <div className="text-sm font-semibold mb-1" style={{ color: "#2D2A2E" }}>
                {item.label}
              </div>
              <div className="text-xs" style={{ color: "#5C5860" }}>
                {item.desc}
              </div>
            </div>
          ))}
      ...
      {/* CTA */}
      <section className="px-6 py-20 text-center max-w-3xl mx-auto">
        <h2 className="text-3xl font-bold mb-4" style={{ color: "#2D2A2E" }}>
          지금 바로 시작해요 🌸
        </h2>
        <p className="text-base mb-8" style={{ color: "#5C5860" }}>
          소중한 40주의 이야기를 아이랑과 함께 써내려가요.
          <br />
          출산 후에도 다이어리북으로 평생 간직할 수 있어요.
        </p>
        <Link
          href="/signup"
          className="inline-block px-10 py-4 text-lg font-semibold rounded-2xl text-white transition-all hover:opacity-90 shadow-lg"
          style={{ backgroundColor: "#FFB4A2" }}
        >
          무료로 시작하기
        </Link>
