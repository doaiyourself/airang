  ...
  return (
    <div className="bg-white rounded-2xl p-5 shadow-sm">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="font-semibold text-base" style={{ color: "#2D2A2E" }}>
          {title}
        </h3>
      </div>
      {children}
...
export function WeeklyContentSection({ content, week }: WeeklyContentSectionProps) {
  const demoContent: WeeklyContent = content ?? {
    ...
    mom_info: {
      body_changes: ["배가 본격적으로 나오기 시작해요", "체중이 늘어나요"],
      common_symptoms: ["허리 통증이 생길 수 있어요", "다리에 쥐가 날 수 있어요"],
    },
    essay: `${week}주차가 됐어요. 매일 조금씩 자라는 아기를 생각하면 신기하고 행복해요. 오늘도 수고 많으셨어요 💕`,
    checklist: ["철분제 챙겨먹기", "정기 검진 예약 확인", "충분한 수분 섭취"],
    exam_info: null,
    ...
    <>
      ...
      <SectionCard emoji="🍼" title={`이번 주 아기 · ${demoContent.baby_info.size_comparison} 크기`} color="#FFF5F3">
        <div className="flex items-center gap-4 mb-3 p-3 rounded-xl" style={{ backgroundColor: "#FFF5F3" }}>
          <div className="text-center">
            <p className="text-2xl font-bold" style={{ color: "#FFB4A2" }}>
              {demoContent.baby_info.length_cm}cm
            </p>
          ...
          <div className="text-center">
            <p className="text-2xl font-bold" style={{ color: "#FFB4A2" }}>
              {demoContent.baby_info.weight_g}g
            </p>
          ...
          <div className="text-center flex-1">
            ...
            <p className="text-xs" style={{ color: "#5C5860" }}>{demoContent.baby_info.size_comparison}</p>
          </div>
        </div>
      ...
      {demoContent.exam_info && (
        <SectionCard emoji="🏥" title="검진 안내" color="#F0F8FF">
          <div className="p-3 rounded-xl" style={{ backgroundColor: "#F0F8FF" }}>
            <p className="font-semibold text-sm mb-1" style={{ color: "#2D2A2E" }}>
              {demoContent.exam_info.name}
            </p>
            <p className="text-xs mb-2" style={{ color: "#5C5860" }}>
              ⏰ {demoContent.exam_info.timing}
            </p>
            <p className="text-xs leading-relaxed" style={{ color: "#5C5860" }}>
              {demoContent.exam_info.purpose}
            </p>
          </div>
        </SectionCard>
      )}
      ...
      <SectionCard emoji="📝" title={`우리의 ${week}주차 기록`} color="#F5EBE0">
        <div className="text-center py-6">
          ...
          <button
            className="mt-3 px-5 py-2.5 rounded-xl text-white text-sm font-medium"
